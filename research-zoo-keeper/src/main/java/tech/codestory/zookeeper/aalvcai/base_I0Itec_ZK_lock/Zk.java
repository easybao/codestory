...
public class Zk implements Lock {
    ...
    public Zk() {
        if (!zkClient.exists(Z_NODE)) {
            zkClient.createPersistent(Z_NODE);//创建持久性节点
        }
    }
    ...
    public synchronized boolean tryLock() {
        ...
        if (path.equals(Z_NODE + "/" + children.get(0))) {
            ...
            int i = Collections.binarySearch(children, path.substring(Z_NODE.length() + 1));//找到自己的下标
            beforePath = Z_NODE + "/" + children.get(i - 1);//找到自己的前一个节点
        }
        return false;
    }
